            ...
            return new Promise((resolve, reject) => {
                ...
                return Promise.sequence(Array.from(configurations.values()).map((configuration, index) => {
                    return () => {
                        ...
                        return new Promise((resolve, reject) => {
                            cp.exec(`PGPASSWORD="${this.config.pass}" pg_dump --schema=${configuration.schema} -s -x --clean -U ${this.config.user} -h ${this.config.host} -p ${this.config.port} --create ${this.config.database}`, {maxBuffer: 1024*1024*20}, (err, stdOut, stdErr) => {
                                if (err) reject(err);
                                else {
                                    const dataFile = path.join(this.dataDir, `${configuration.index}-${configuration.schema}.sql`);
                ...
                return Promise.sequence(Array.from(configurations.values()).filter(c => c.dataFrom && c.dataFrom.length).map((configuration) => {
                    return () => {
                        ...
                        return new Promise((resolve, reject) => {
                            cp.exec(`PGPASSWORD="${this.config.pass}" pg_dump --schema=${configuration.schema} -a -t '"${configuration.schema}"."${configuration.dataFrom.join(`"' -t '"${configuration.schema}"."`)}"' -x -U ${this.config.user} -h ${this.config.host} -p ${this.config.port} --create ${this.config.database}`, {maxBuffer: 1024*1024*20}, (err, stdOut, stdErr) => {
                                if (err) reject(err);
                                else {
